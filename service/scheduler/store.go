...
func Create(db *store.DB, s *Schedule) (int64, error) {
	...
	res, err := db.Exec(
		db.Rebind(`INSERT INTO schedules (type, cron_expr, scheduled_at, task, channel, channel_meta, timezone, description, enabled, model_tier, max_budget_usd)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),
		string(s.Type), s.CronExpr, scheduledAt, s.Task, s.Channel, string(metaJSON), s.Timezone, s.Description, 1, s.ModelTier, s.MaxBudgetUSD,
	)
	if err != nil {
...
func Get(db *store.DB, id int64) (*Schedule, error) {
	row := db.QueryRow(
		db.Rebind(`SELECT id, type, cron_expr, scheduled_at, task, channel, channel_meta, timezone, description, enabled, last_run_at, last_error, created_at, completed_at, model_tier, max_budget_usd
...
func List(db *store.DB) ([]Schedule, error) {
	rows, err := db.Query(
		`SELECT id, type, cron_expr, scheduled_at, task, channel, channel_meta, timezone, description, enabled, last_run_at, last_error, created_at, completed_at, model_tier, max_budget_usd
