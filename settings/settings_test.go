...
func TestXStorageDriverField(t *testing.T) {
	cfg := config.Default()
	svc := testService(cfg)

	field := findField(svc, "x.storage.driver")
	if field == nil {
		t.Fatal("x.storage.driver field not found")
	}

	got := svc.GetValue(field)
	if got != "sqlite" {
		t.Errorf("default storage driver = %q, want sqlite", got)
	}

	if err := svc.SetValue(field, "postgres"); err != nil {
		t.Fatal(err)
	}
	if got := svc.GetValue(field); got != "postgres" {
		t.Errorf("after set, storage driver = %q, want postgres", got)
	}
}

func TestXStorageDSNField(t *testing.T) {
	cfg := config.Default()
	svc := testService(cfg)

	field := findField(svc, "x.storage.dsn")
	if field == nil {
		t.Fatal("x.storage.dsn field not found")
	}

	got := svc.GetValue(field)
	if got != "" {
		t.Errorf("default DSN = %q, want empty", got)
	}

	if err := svc.SetValue(field, "postgres://localhost/x"); err != nil {
		t.Fatal(err)
	}
	if got := svc.GetValue(field); got != "postgres://localhost/x" {
		t.Errorf("after set, DSN = %q, want postgres://localhost/x", got)
	}
}
