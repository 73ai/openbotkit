...
func installSkills(t *testing.T, dir string) {
	...
	for _, name := range []string{"email-read", "whatsapp-read", "memory-read", "memory-save", "contacts-search", "whatsapp-send", "email-send", "schedule-task", "web-search", "web-fetch", "x-read", "x-post"} {
